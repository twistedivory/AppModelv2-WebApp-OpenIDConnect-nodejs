...
function Strategy(options, verify) {
  ...
  options.userInfoURL = options.userInfoURL || 'https://login.microsoftonline.com/common/oauth2/v2.0/userinfo';
  options.revocationURL = options.revocationURL || 'https://login.microsoftonline.com/oauth2/v2.0/oauth2/logout';
  ...
  if (req.query && req.query.error) {
    ...
    this.configure(null, function(err, config) {
      ...
      oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
        ...
        log.info("OPENID CONNECT: We are in the OpenID Connect Response. Assumed because no auth code was in the query and we are POST.")
        log.info("");
        ...
        var profile = {};
            ...
            if (self._passReqToCallback) {
                self._verify(req, iss, sub, jwtClaims, profile, verified);
              } else {
                self._verify(iss, sub, jwtClaims, profile, verified);
              }
            }
